			...
			if (console && console.log)
			{
				if (info.initializingCall)
				{
					...
					if (info.namespace === window && info.specialFields.length !== 0)
					{
						...
						if (info.specialNonFunctions.length !== 0)
						{
						    console.log("Found " + info.specialNonFunctions.length + " non-function special field" + pluralS(info.specialNonFunctions.length) + ": " + truncate(info.specialNonFunctions.join(", "), 280));
						}
					}
		...
		{	
			...
			for (var fieldName in namespace)
			{
				...
				if (typicalFields.indexOf(fieldName) === -1)
				{
					...
					if (typeof namespace[fieldName] !== "function")
					{
					    info.specialNonFunctions.push(fieldName);
					}
				}
